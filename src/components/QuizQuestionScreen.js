...
const QuizQuestionScreen = (props) => {
    ...
    if( status === 'quizEnded') {
        return (
            <QuizQuestion>
                <p className="answer_block">{`Correct Answers: ${correctAnswer} out of ${Questions.length}`} <button onClick={ () => window.location.reload() } className="button">Take Test again</button></p>
            </QuizQuestion>
        )
    }
